...
# Folder and filename
rtp_name = ('0217_run1_fashionmnist')
# Dataset
...
rtp_discriminator_n_embedding = 50
# Generator parameters
rtp_generator_n_embedding = 50
# Training parameters
rtp_train_fid = 10
...
def train(g_model, d_model, gan_model, dataset, latent_dim, fid_model, n_epochs=rtp_train_n_epochs, n_batch=rtp_train_n_batch):
	...
	for i in range(n_epochs):
		...
		fid_samples_fake = zeros((sample_per_epo, half_batch, 28, 28, 1))
		fid_samples_fake = fid_samples_fake.astype('float32')
		fid_samples_real = zeros((sample_per_epo, half_batch, 28, 28, 1))
		...
		for j in range(bat_per_epo):
			...
			if (j % fid_mod >= fid_mod-sample_per_epo):
			...
			# Write to file
			if (j % bat_per_epo == 0):
				file_str = '%.3f, %.3f, %.3f, %.03f\n' % (d_loss1, d_loss2, g_loss, fid)
			else:
				file_str = '%.3f, %.3f, %.3f\n' % (d_loss1, d_loss2, g_loss)
			with open('results_csv.txt', 'a') as file:
				file.write(file_str)
		...
		fid_samples_fake = reshape(fid_samples_fake, [sample_per_epo*half_batch, 28, 28])
		fid_samples_real = reshape(fid_samples_real, [sample_per_epo*half_batch, 28, 28])
		...
		print(" ->->-> FID for epoch %d: %.03f" % (i, fid))

		# save the generator model
		f_name = rtp_name + '_%d.h5' % (i + 1)
		g_model.save(f_name)
...
#overwrite csv files
with open('results_csv.txt', 'w') as file:
	file.write('')
