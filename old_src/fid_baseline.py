...
def train(g_model, d_model, gan_model, dataset, latent_dim, fid_model, n_epochs, n_batch=128):
	...
	for i in range(n_epochs):
		...
		f_name = '0215_cgan_bal_reference%d.h5' % (i + 1)
		g_model.save(f_name)
...
for i in range(0, n_tests):
	[x1, _], _ = generate_real_samples(dataset, b_size//2)
	[x2, _], _ = generate_real_samples(dataset, b_size//2)
	...
	sum += calculate_fid(fid_model, x1, x2)
	print("i: %d\tsum: %.03f\tmean: %.03f" % (i, sum, sum/(i+1)))

print("\nDONE!\nsum: %.03f\tmean: %.03f" % (sum, (sum/n_tests)))